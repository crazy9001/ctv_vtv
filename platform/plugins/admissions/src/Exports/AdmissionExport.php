...
namespace Botble\Admission\Exports;
...
class AdmissionExport extends TableExportHandler
{
    /**
     * {@inheritDoc}
     */
    protected function afterSheet(AfterSheet $event)
    {
        ...
        for ($index = 2; $index <= $totalRows; $index++) {
            ...
            if ($value == AdmissionStatusEnum::READ) {
            ...
            $event->sheet
                ->getDelegate()
                ->getCell('F' . $index)
                ->setValue(AdmissionStatusEnum::getLabel($value));
        }
    }
}
