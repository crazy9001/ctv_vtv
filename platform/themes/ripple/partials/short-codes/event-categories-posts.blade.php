@if (is_plugin_active('blog'))
    @php
      $featured = get_posts_by_category($category);
    @endphp
                                ...
                                <article class="post post__inside post__inside--feature">
                                    <div class="post__thumbnail">
                                        <img
                                            ...
                                            alt="{{ $featureItem->name }}"><a href="{{ $featureItem->url }}"
                                                                              class="post__overlay"></a>
                                    </div>
                                    <header class="post__header">
                                        <h3 class="post__title"><a
                                                href="{{ $featureItem->url }}">{{ $featureItem->name }}</a></h3>
                                        <div class="post__meta"><span class="post-category"><i class="ion-cube"></i>
                                                @if (!$featureItem->categories->isEmpty())<a
                                                    href="{{ $featureItem->categories->first()->url }}">{{ $featureItem->categories->first()->name }}</a>@endif
                                        </span>
                                            ...
                                            @if ($featureItem->author->getFullName())
                                                <span class="post-author"><i
                                                        class="ion-android-person"></i><span>{{ $featureItem->author->getFullName() }}</span></span>
                                            @endif
                                        </div>
                                    </header>
                                </article>
